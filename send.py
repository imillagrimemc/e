...
def sch(message):
    if message.contact is not None:  # Если присланный объект <strong>contact</strong> не равен нулю
        ...
        cursor = connect.cursor()
        cursor.execute(
            "UPDATE  users SET phone =" + str(message.contact.phone_number) + " WHERE id=" + str(message.chat.id))
        connect.commit()
...
def cotb(message):
    ...
    connect.commit()
    bot.send_message(message.chat.id, text="Категория вашего товара, ассортимент (максимально подробно, пожалуйста)",
...
def site(message):
    ...
    connect.commit()
    bot.send_message(message.chat.id,
                     text="АКТИВНЫЕ ссылки на социальные сети (т.е. не просто название аккаунта @sample, а именно активная ссылка) и сайт, если есть",
                     reply_markup=types.ReplyKeyboardRemove())
...
def callback_inline(call):
    if call.data == 'tovar_1':
        ...
        cursor.execute(
            "UPDATE users SET stelajone=?, stelajtwo=?, stelajthree=?, chair=?, hanger=?, dummy=?, mirror=?, storage=?, electric=? WHERE id=?",
            (0, 0, 0, 0, 0, 0, 0, 0, 0, call.message.chat.id))
        # Сохраняем изменения
        connect.commit()
